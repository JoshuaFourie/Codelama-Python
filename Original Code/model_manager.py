...
class MultiModelManager:
    ...
    def load_model(self, language, hf_token=None):
        """
        Lazy load a model optimized for Ryzen 7800X3D and RTX 4070
        - Optimized for CodeLlama-13B-Instruct
        ...
        # For CodeLlama-13B-Instruct, always use 4-bit quantization
        print(f"Loading {language} model with PyTorch (optimized for RTX 4070)...")
        print(f"Using 4-bit quantization for {model_name}")
        
        # Load the model with 4-bit quantization optimized for CodeLlama-13B-Instruct
        model = AutoModelForCausalLM.from_pretrained(
            model_name,
            **auth_kwargs,
            cache_dir=self.cache_dir,
            quantization_config=self._configure_quantization(
                optimize_for_rtx40=self.gpu_info.get('rtx40_series', False)
            ),
            device_map="auto",
        ...
        print(f"{language} model loaded successfully using PyTorch.")
        return model, tokenizer
   
    def _optimize_memory(self):
        """
        Optimize memory usage before loading new models
        """
    ...
    def generate_code(self, prompt, language=None, chat_history=None,
                      temperature=0.2, max_new_tokens=1024, 
                      repetition_penalty=1.1):
        """
        Generate code with hardware-optimized inference
        - Optimized for CodeLlama-13B-Instruct
        """
        ...
        # Format the prompt using the model's template if available
        prompt_template = self.models_config[language].get('prompt_template')
        if prompt_template:
        ...
        try:
            ...
            yield from self._generate_with_pytorch_safe(
                model, tokenizer, formatted_prompt, 
                temperature, max_new_tokens, repetition_penalty
            )
    ...
    def _generate_with_pytorch(self, model, tokenizer, prompt, 
        ...
        with torch.no_grad(), torch.cuda.amp.autocast():  # Use automatic mixed precision
            # Use RTX 4070-optimized settings with CodeLlama-specific adjustments
            generated_ids = model.generate(
                ...
                top_k=50,        # Limit sampling to top 50 tokens for efficiency
                top_p=0.95,      # Use nucleus sampling for better code quality
                num_beam_groups=1,  # Single beam group is faster
                num_beams=1,     # Disable beam search for faster generation
                # Improved settings for 7800X3D's 3D V-Cache
                early_stopping=True,  # Stop generation when all sequences reach eos_token_id
